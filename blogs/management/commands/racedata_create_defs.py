...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        weekday = options['week'][0]
        print("Week:{}".format(weekday))
        raceobs = Racelist.objects.filter(date__week_day = weekday).filter(date__gt = five_days_ago).filter(racedata_updated_at__lt = time_threshold)

        all_race_list_len = len(Racelist.objects.filter(date__week_day = weekday).filter(date__gt = five_days_ago))
        print("Remain race is {}/{}".format( len(raceobs), all_race_list_len))
        #raceobs = Racelist.objects.all()
        for race in raceobs:
            print( "{}/{}".format(len(Racelist.objects.filter(racedata_updated_at__lt = time_threshold)), all_race_list_len) )
            print(race.url)
